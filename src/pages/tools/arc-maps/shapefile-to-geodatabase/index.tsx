...
export default function ShapefileToGeodatabase() {
  ...
  const handleDownloadGdbZip = async () => {
    ...
    try {
      ...
      gdbFolder.file('gdb_manifest.json', JSON.stringify({
    ...
    <Paper sx={{ p: 3, maxWidth: 900, mx: 'auto', mt: 2 }}>
      <Stack spacing={2} mb={3}>
        ...
        <Typography variant="body2" color="text.secondary">
          Convert ESRI Shapefile vector packages (.shp, .dbf, .zip) into File Geodatabase (.gdb) spatial containers for ArcGIS 10.8 & ArcGIS Pro.
        </Typography>
      </Stack>
            ...
            {geoResult ? (
              <>
                ...
                <Box mt="auto">
                  ...
                  >
                    {isProcessing ? 'Generating Geodatabase...' : 'Download File Geodatabase (.gdb.zip)'}
                  </Button>
                </Box>
              </>
            ) : (
              <Box display="flex" alignItems="center" justifyContent="center" height={300} color="text.secondary">
                Upload Shapefile (.shp / .zip) to construct File Geodatabase container.
              </Box>
            )}
          </Paper>
        </Grid>
