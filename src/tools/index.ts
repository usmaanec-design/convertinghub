...
import { arcMapsTools } from '../pages/tools/arc-maps';
...
const categoriesConfig: {
...
}[] = [
  {
    type: 'arc-maps',
    icon: 'gis:earth',
    value: 'translation:categories.arc-maps.description',
    title: 'translation:categories.arc-maps.title'
  },
  {
    type: 'string',
...
export const filterToolsByUserTypes = (
  ...
  const matched = tools.filter((tool) => {
    if (tool.userTypes && tool.userTypes.length > 0) {
      return tool.userTypes.some((userType) => userTypes.includes(userType));
    }
    const catMapping = CATEGORIES_USER_TYPES_MAPPINGS[tool.type];
    if (catMapping) {
      return catMapping.some((ut) => userTypes.includes(ut));
    }
    return true;
  });
...
export const getToolsByCategory = (
  ...
  const groupedByType: Record<string, DefinedTool[]> = {};
  ...
  return toolCategoriesOrder
    .map((type) => {
      const categoryTools = groupedByType[type] ?? [];
      ...
      const filteredTools =
        userTypes.length > 0
          ? filterToolsByUserTypes(categoryTools, userTypes)
          : categoryTools;
      ...
      return {
        ...
        type,
        icon: categoryConfig?.icon ?? 'gis:earth',
        tools: filteredTools,
        example:
          filteredTools.length > 0
            ? { title: filteredTools[0].name, path: filteredTools[0].path }
            : { title: '', path: '' },
        userTypes: aggregatedUserTypes
      };
    })
